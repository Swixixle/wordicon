...
MUTATIONS = [
    ...
    {"name": "discard an envelope based on an older revision than the head (the review's finding 1, as delivered)", "file": "webapp/work/session.js",
     "old": "    if (chosen && chosen.base_revision !== d.revision) {\n      const res = await this.reconcile(chosen, d);   // never throws; never discards\n      if (res.kind === 'rebased') chosen = res.envelope;\n      else { unreconciled = chosen; chosen = null; }\n    }",
     "new": "    if (chosen && chosen.base_revision !== d.revision) { chosen = null; }",
     "expect": "after the reload every later word is in the editor"},
    {"name": "take the globally newest envelope instead of this tab's own (the review of 0f2db31, finding 1, as delivered)", "file": "webapp/work/session.js",
     "old": "    const mine = envs.find(e => e.tab_id === this.tab) || null;",
     "new": "    const mine = envs.filter(unsent).sort((a, b) => (b.at || '').localeCompare(a.at || ''))[0] || null;",
     "expect": "tab A reopens with ITS OWN unsent words"},
    {"name": "mark another tab's envelope abandoned once it was shown here", "file": "webapp/work/session.js",
     "old": "      if (takenFrom) { try { await recovery.write({ ...takenFrom, recovered_by: this.tab, recovered_at: new Date().toISOString() }, { keepAt: true }); } catch (e) {} }",
     "new": "      for (const f of foreign) { try { await recovery.write({ ...f, abandoned: true, taken_by: this.tab }); } catch (e) {} }\n      if (takenFrom) { try { await recovery.write({ ...takenFrom, recovered_by: this.tab, recovered_at: new Date().toISOString() }, { keepAt: true }); } catch (e) {} }",
     "expect": "neither overwritten, abandoned nor marked carried"},
    {"name": "call an envelope the head when only its words and structure match (an unsaved rename lost)", "file": "webapp/work/session.js",
     "old": "      if ((e.title || '') !== (d.title || '') || !!e.title_is_manual !== !!d.title_is_manual) return false;\n",
     "new": "",
     "expect": "the unsaved rename is recovered and saved"},
