...
(async () => {
  ...
  ok(/Deployment runs the pinned revision\s*not ruled/.test(eaCard) && /test mode: a declared development connector on loopback/.test(eaCard) && /pinned from the producer’s source at 1a71460/.test(eaCard), 'the EthicalAlt card says no ruling was made that the deployment runs the pinned revision, that starting works here only against the fixture producer, and which revision the contract was read from');
  ...
  ok(/Kept from the producer: the producer['’]s unsigned structured export kept \(\d+ bytes, sha256 [0-9a-f]{12}/.test(invCard), 'the unsigned export is kept with its size and hash: ' + (invCard.match(/export kept \([^)]+\)/) || [''])[0]);
  ok(/Signed receipt: signature verified \(ethicalalt\.receipt\.v1 \(stableStringify \+ Ed25519\)\) under the pinned key/.test(invCard) && /investigation_id is this operation’s|investigation_id is this operation's/.test(invCard), 'the receipt’s signature verified under the pinned key and correlates by investigation id');
