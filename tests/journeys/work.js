...
(async () => {
  ...
  ok(/Runs · \d+/.test(yw) && /\d+ items? searchable/.test(await page.textContent('#work-health')) && /items from \d+ stores, generation \d+/.test(await page.textContent('#work-about')), 'and the runs, with the index’s own population line: ' + (await page.textContent('#work-about')).slice(0, 60));
  const cardWords = await page.evaluate(() => Array.from(document.querySelectorAll('#work-list .work-item')).map(c => c.querySelector('.result-title').textContent + ' ' + c.querySelector('.muted.small-text').textContent).join(' | '));
  ok(cardWords.length > 0 && !/\b(investigate|explore|write|edit|more)\.[a-z]+\.[a-z]+|\b(explore|write)\.[a-z]+\b/.test(cardWords), 'the cards show registry labels, not action ids: ' + cardWords.slice(0, 100));
  ...
  ok(/Live use\s*not enabled/.test(eaCard) && /test mode: a declared development connector on loopback/.test(eaCard) && /read from the producer’s own source/.test(eaCard) && !/1a71460/.test(eaCard.split('The contract, as pinned')[0]), 'the EthicalAlt card says live use is not enabled, that starting works here only against the fixture producer, that the contract is read from the producer’s source — and keeps the commit identifier out of its face');
  ok(/start: POST \/api\/investigate · auth none · server\/routes\/tap\.js:506/.test(eaCard) && /NOT served by main: \/api\/profiles\/\{slug\}\/export\/v2/.test(eaCard), 'the contract details name each route with its file:line in the producer’s source, and say the package export is not served by main');
  ok(/Cost\s*unknown here/.test(eaCard), 'the cost is said on the card: unknown here, billed on the producer’s side');
  ok(/not served by either producer’s main branch/.test(await page.textContent('#package-contract-note')), 'the page says the rooms’ package route is not served by main at the pinned revisions');
  ...
  ok(/Producer: ethicalalt · upstream id nk_/.test(invCard) && /Start: ok · delivery: delivered/.test(invCard), 'the investigation card names the producer, the upstream id, the start’s outcome and its delivery');
  ok(/Kept from the producer: the producer['’]s unsigned structured export \(\d+ bytes\)/.test(invCard) && /export\.json sha256 [0-9a-f]{64}/.test(invCard), 'the unsigned export is kept with its size on the card and its hash in the details: ' + (invCard.match(/unsigned structured export \([^)]+\)/) || [''])[0]);
  ok(/Signed receipt: verified under the key you pinned — the receipt for this request/.test(invCard) && /signed record: signature verified \(ethicalalt\.receipt\.v1 \(stableStringify \+ Ed25519\)\) under the pinned key/.test(invCard) && /investigation_id is this operation’s|investigation_id is this operation's/.test(invCard), 'the receipt’s verification is said plainly on the card, with the method and key in the details, and correlates by investigation id');
  ok(/start-reply\.json · \d+ bytes/.test(invCard) && /export\.json · \d+ bytes/.test(invCard) && /receipt\.json · \d+ bytes/.test(invCard), 'the three replies are kept byte for byte and shown as chips');
  ok(/A valid signature says the bytes are the producer’s under the key you pinned — not that the research is true/.test(invCard), 'and the card says what a signature does and does not mean');
  ...
  ok(JSON.stringify(invKinds) === JSON.stringify(['stage_intent:POST', 'stage_end:POST=ok', 'stage_intent:GET ', 'stage_end:GET =ok', 'stage_intent:POST', 'stage_end:POST=ok']), 'the record holds the intent before and the outcome after the start, the export’s read, and the receipt’s request: ' + JSON.stringify(invKinds));
