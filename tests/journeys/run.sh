...
for need in "Find related words is the first door in Explore this idea" "opening the door spent nothing" "the saved comparisons for this idea are shown before anything is spent" "a comparison matched by concept id and one matched only by title are labelled apart" "one press started exactly one run, with no second form" "the meaning that went is the one he edited, not the card's" "a close synonym and a related one are told apart" "an exact antonym and a contrast are told apart, and each says what it opposes" "a non-Latin word offered as an English synonym is set aside, visibly, with its reason" "Latin has its own section, with the term and its period" "Greek has its own section: the period name, the Greek letters, the romanization, the period" "a language that came back empty reads as no close match in this pass, not as absence" "the follow-up rendered under the language it was asked for" "and the results above it were not replaced" "its English sections say they were never part of the tool then — not that nothing was found" "Latin and Greek say they were not asked for, by name" "the bar is still four buttons and none of them is Find related words" "and spends nothing" "a selected word is the meaning line as it stands" "the run needs no title and no accepted concept" "the selection went as the passage, for context, exactly" "the draft is untouched by the run" "and never calls it a workup" "every result carries Explore this word, Compare with my idea and Save" "Compare sets your meaning beside the word's own meaning, fit and differences, with the review" "a field the pass did not produce is named as not analysed, and the English sections say they were not reviewed" "Explore this word opens the word-comparison panel with the word, its sense and the cost, and waits for Find" "Compare and Explore sent no run" "saving is one append and no run" "the saved word carries its run, language, intended meaning and review" "the Library has a Saved words shelf with the word, its language and its verdict" "remove takes it off the shelf"; do
  grep -qF "ok   $need" "$JOURNEY_OUT/related.log" || { echo "== related: missing check: $need"; status=1; }
done
total=$(grep -h "^CHECKS " "$JOURNEY_OUT"/quiet.log "$JOURNEY_OUT"/home.log "$JOURNEY_OUT"/anatomy.log "$JOURNEY_OUT"/chooser.log "$JOURNEY_OUT"/speak.log "$JOURNEY_OUT"/speakkeep.log "$JOURNEY_OUT"/encounter.log "$JOURNEY_OUT"/federation.log "$JOURNEY_OUT"/shell.log "$JOURNEY_OUT"/room.log "$JOURNEY_OUT"/deep.log "$JOURNEY_OUT"/resume.log "$JOURNEY_OUT"/inquiry.log "$JOURNEY_OUT"/epistemic.log "$JOURNEY_OUT"/partial.log "$JOURNEY_OUT"/constitution.log "$JOURNEY_OUT"/anchorfit.log "$JOURNEY_OUT"/carry.log "$JOURNEY_OUT"/wayfinder.log "$JOURNEY_OUT"/map.log "$JOURNEY_OUT"/moira.log "$JOURNEY_OUT"/notebook.log "$JOURNEY_OUT"/related.log | awk '{s+=$2} END {print s+0}')
