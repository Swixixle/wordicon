...
|---|---|---|---|
| A — fail-closed test environment, explicit data root, baseline ledger | done | (this commit) | suite exit 0 with the new canary; 25 journeys, 983 checks, exit 0, under test mode; sabotage of the canary (two ways) fails it by name |
| B — /work shell, action registry, notebook + fixture result path | not started | | |
...
- Journeys: `JOURNEY_DIR=<scratch> bash tests/journeys/run.sh` (≈10 min; `== journeys: 983 checks, exit 0` at `17894e3` + slice A).

## Remaining, in order

B, C, D, E, F, G as the instructions' §4 table. Blockers: none for the local workspace. External deployment facts (EthicalAlt, Open Case, PUBLIC EYE, the Rabbit Hole repository) are unavailable and block only live verification of their adapters.
