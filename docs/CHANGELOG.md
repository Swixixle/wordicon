...
## v1.36.9 — the front door shows the workspace (documentation only)

No application source changed; one screenshot script was added under `tests/slice_g/`. The README's "What it looks like" now shows the writing workspace at `/work` as it renders at `fff87d3` — the draft with Tools and Results, the selection menu, a proposal before Start, a signed result beside the draft, Your work, another tab's unsaved copy listed above the draft, phone width — and says where the older desk still is; "First use" starts in the workspace and names `scripts/preview.py` as the way to try a candidate on its own root. `docs/screenshots/workspace-v2/` is retaken at `fff87d3` (25 kept, 92 measured in Chromium and WebKit, none with overflow or a page error; the index records the commit), with four views of the copies bar and the Investigate authorization control taken by the new `tests/slice_g/shots_drafts.js`, which is the script the Report 86 archive ran. Every image is the application on synthetic fixtures; nothing of the owner's writing appears in any of them.

## v1.36.8 — workspace-v2 slice I: the review of 0f2db31, corrected

Two tabs of one browser with different unsaved words are two versions: a reopening tab gets its own words back (reconciled as before), and every other tab's version is listed above the draft — kept whole, never overwritten, never marked consumed by being seen — to become its own document, fill an empty editor, or be deleted by a press; a gone tab's words can fill an empty editor and are left in place until saved; an unsaved rename is no longer mistaken for the saved head. A preview on a copied store dispatches nothing it inherited (queued work stays queued, marked inherited and paused; a press makes a linked attempt) and the copy holds the source's lease throughout, failing closed when it cannot be checked. An investigation interrupted after its request left reads as delivery unknown, never "not sent"; a producer's reply for another record is kept as evidence and never followed; a correctly signed snapshot or receipt for another record is said to be exactly that — signature valid, not the record requested — and the unsigned report's identity is said to be checked only by the case id it names; recovery controls are the producer's own acts. The cards say what decides in plain words and keep commit identifiers, methods and key ids in their disclosures; the owner authorizes live use, the implementer verifies deployments.

## v1.36.7 — workspace-v2 slice H: the review of 6e5b59c, corrected

Finding 1: a save that committed on the server but whose reply was lost could, on reopening, discard every word typed after it and overwrite their recovery copy; the envelope's pending request is now resent under its own id and the divergence reconciled through the server's `repeated` answer — rebased when that save made the head, both versions kept as a named conflict when another writer advanced it, never combined by guesswork; proven in both engines for a body edit on a same-tab reload, a formatting-only edit from a fresh tab, and a head that moved on, and by two sabotage mutations caught by name. Finding 2: a candidate started on a copied store would have sealed into and pruned the owner's real backups through the copied Vault configuration and history; `WORDICON_PREVIEW=1` stands the Vault down process-wide (no seal at start, on the timer or at shutdown, no prune, no drill, no notification; the Home strip and the workspace say PREVIEW), and `scripts/preview.py` is the one way to try a candidate — a new root by exclusive `mkdir`, never reused, nothing deleted, the source's lease respected, `vault/`, `auth/`, locks and the derived index left behind, SQLite stores copied through the backup API, loopback only, fixtures unless `--live`; the report's copy/delete recipe is withdrawn; the hazard and the guard are both proven by `_check_preview_isolation`. Finding 3: the adapters expected the signed package routes, which neither producer's main serves (they exist on unpushed `block107` branches only); `scripts/producers.py` now pins EthicalAlt `1a71460` and Open Case `4dc1709` with file:line per route and implements what main serves — EthicalAlt's index, start, UNSIGNED export (kept with its hash, never called a signed record) and signed receipt (verified here under the pinned key by the producer's own `stableStringify` + Ed25519, re-implemented byte for byte and proven against Node-written vectors and a Node-signed receipt; the reply's key compared, never trusted; a cached receipt named as the earlier request's); Open Case's case list, start (subject required; the handle must be the key holder's), report (a read the producer counts) and the signed snapshot as its own disclosed mutation, verified as JCS → sha256 hex → Ed25519; delivery kept apart from outcome (a 4xx refused; a 5xx, an HTML page, bad JSON or a redirect after delivery leaves the outcome UNKNOWN); recovery says what it read again; the ruling worded and recorded as "the deployment runs the pinned revision"; live starts stay disabled. Also: a repeated save request under a different intent is refused by name (a versioned request fingerprint); the gate's master secret is minted by exclusive create (twenty threads and eight processes agree on one; an empty secret is refused); Word export from the structure; the concept picker from the shelf; registry labels on Your work and the index's internals in a disclosure; the investigation card says what was kept, the delivery, and what a signature means.

## v1.36.6 — workspace-v2 slice G: parity, accessibility, failure testing, delivery

The races the instructions name, proven and fixed: a document switch waits (bounded) for the save in flight and sends the words typed during it; a reply that lands after the switch stopped waiting repairs its own envelope, so nothing typed is lost and a reopening recovers exactly what was never acknowledged; reopening a document whose envelope holds exactly the head recovers nothing and rewrites nothing (before this, a plain document reopened after a dropped acknowledgment was silently made structured); two tabs on one document keep both copies; an application made while a save is in flight is committed by the save that carries it and its undo by the next; a browser whose storage refuses never claims a copy it does not have. Twenty identical Starts are one operation and one dispatch; a second process contending for the dispatcher is refused by name. The Vault copies the SQLite stores through SQLite's own backup API inside its drained window and counts their rows in the manifest — a writer outside the corpus lock cannot tear them; a backup taken with a writer mid-transaction restores consistent. The constitution gains the workspace's clause, filed as the build instruction that made it: the sides act only on a press and never insert or replace writing, accept or rule on a result, or initiate research on their own; the desk's rule stands for `/`. The shell keeps both panels on a 1280 px laptop by narrowing them, wraps the formatting bar under 1180 px instead of overlapping, carries the Results tabs into the section-below arrangement, keeps Hide in reach on a narrowed side, and every control is reachable by Tab (the grips resize on the arrow keys); a backward selection, the scroll position and a pending save survive the side toggles and a reading's arrival never takes the caret; two chrome tokens moved so every text pair clears 4.5:1. A sabotage pass of nineteen mutations (server and browser) each failed its named check and was restored; two found weak tests and made them stronger (the origin pin's second layer; the third attempt's background run). Screenshots of the running workspace at every arrangement, zoom and width, in Chromium and WebKit, are under `docs/screenshots/workspace-v2/`; the migration/backup/restore/rollback rehearsal on disposable data is in the delivery report.

## v1.36.5 — workspace-v2 slice F: investigation adapters and Ask

Each instrument has an adapter that declares the contract it was built against — route, method, auth reference, payload, side effects, limits, id correlation — and where that contract came from. None has been verified against a deployment from this workspace, so the adapter disables live starts until the owner records a verification ruling on the connector; in test mode a declared development connector on loopback may start, so the mechanism is proven on the fixture producer. Readiness is derived per capability from the record and opening Investigate contacts nothing. A start is a proposal first (what you named, what leaves, to whom); then one origin-pinned request at the dispatch boundary with its intent and outcome and the upstream id; then the signed export read into custody through the existing verifier under the pinned key, its signature state kept apart from the live reply — a valid signature says the bytes are the producer's, not that the research is true. A producer's error is a known failure, an unreachable producer an unknown outcome, a missing credential a refusal before anything is sent; the credential's value is never in the record. Ask takes a plain request like "investigate Exemplar Holdings" and proposes with its rest as the subject.

## v1.36.4 — workspace-v2 slice E: Your work, the derived index

Your work searches everything the record keeps — writing, runs and their inputs, readers' readings, revision notes, concepts, saved words, questions, sources, recordings, rooms, depositions, inquiries, recovery cases, operations — through a derived SQLite/FTS5 index that is never an authority: every item names its store, its kind and its native id and says how to reopen the record it came from. The notebook reaches the index through an outbox row written in the save's own transaction (a crash between saving and indexing is repaired; an index failure never turns a save into a failure); file stores are read back from the authoritative file when their size or mtime changes; the operations store through its event sequence. Rebuilds land in a new generation and a failed rebuild keeps the last working one. The user's words are bound as parameters and quoted as literal terms — a malformed query is answered, never a crash — and the health line says "Search is updating; some recent work may be missing" instead of an exhaustive no. Two identical documents stay two items; a run and a document are linked only by a recorded operation, never by similarity. Archive is a ruling that hides an item from ordinary listings, keeps it whole, and has its own filter.

## v1.36.3 — workspace-v2 slice D: durable operations and one dispatcher

Every operation the application starts is a row in a SQLite store under the state root before anything runs: reserved under a request key with the fingerprint of the full execution request (the same key returns the same operation; a changed request under the same key is refused), claimed by one atomic transition, dispatched only by the process holding the dispatcher lock, and followed by an ordered record of events — a dispatch intent persisted before every stage and every real HTTP attempt, its outcome after, the result's ids, completion or failure. After an interruption the record says which of six cases it is in, from evidence: never dispatched (resumed under the unchanged plan), delivery unknown (never sent again), the reply received and the result missing (unknown), a result persisted and the link missing (reconciled by its stable id). A complete operation reopens after a restart from the store and the result files, on the workspace route and on the legacy job route. "Check status / recover result" reads and sends nothing; "Start another attempt" is a new operation under a new key, linked, refused while the first still runs and refused by name when the lane, model, registry or action changed. Result files are written whole (temporary file, atomic link) under their stable ids. Proven in the suite on the store, on a flaky gateway, and on two real killed processes; in the workspace journey on two interrupted operations the scratch server reconciles at startup.

## v1.36.2 — workspace-v2 slice C: the structured editor and the storage contract

The workspace's editor is ProseMirror from a pinned local bundle (no CDN), on the same blue surface with the same yellow text. The exact text of a document is the projection of its structure — leaf paragraphs joined by two line feeds, a hard break one, nothing appended — and the server computes that projection itself and refuses a structure that does not match the text. Formatting is a versioned edit under a new fingerprint form; old fingerprints are not rehashed. Opening a plain document does not rewrite it; the first formatted save keeps the plain text as a version and records the conversion. A document with carriage returns opens read-only and exact with a disclosed conversion; other control characters keep the plain editor. An old client cannot flatten a formatted document (refused by name, explicit null included), and the database refuses an older binary's body-only write so a rollback keeps everything. Versions can be restored as a new revision; a plain copy is a new document; text export is visibly plain, Markdown keeps the structure, Print makes a PDF. A candidate word from a selection can be put into the draft — insert below or replace — only while the draft is as it was when the result was made, as one undo step, recorded as an event and marked committed by the save that carries it; a stale result needs its target chosen again. Find and replace; word count; the formatting bar keeps the editing selection; a composition (IME, dead key) finishes before a save or a capture. Recovery keeps the whole structured document, including a document the server never received. Two shell bugs fixed: every click in the shell had its default action prevented (details could not open); a find recount after typing moved the caret onto a match. Tests: `_check_document_contract`; `tests/journeys/editor.js` in Chromium and in WebKit; `work.js` moved off the textarea with an old→new map.
